...
pdsh -w 192.168.0.[141-145] "weka local setup container --name drives0 --base-port 14000 --cores 2 --only-drives-cores --net eth1 --failure-domain `hostname` --memory 0"
...
pdsh -w 192.168.0.[141-145] "weka local setup container --name compute0 --base-port 15000 --cores 1 --only-compute-cores --net=eth2 --memory 11GiB --failure-domain `hostname` --memory 0"
...
pdsh -w 192.168.0.[141-145] "weka local setup container --name frontend0 --base-port 16000 --cores 1 --only-frontend-cores --net=eth3 --failure-domain `hostname` --memory 0"

# ? Needed?
for i in {0..4}; do weka cluster container net add $i eth1 --netmask=24 --gateway=192.168.1.1; done
for i in {0..4}; do weka cluster container net add $i eth2--netmask=24 --gateway=192.168.2.1; done
for i in {0..4}; do weka cluster container net add $i eth3 --netmask=24 --gateway=192.168.2.1; done
...
weka cluster create weka4-node01 weka4-node02 weka4-node03 weka4-node04 weka4-node05 \
--host-ips=192.168.0.141,192.168.0.142,192.168.0.143,192.168.0.144,192.168.0.145


# for i in {0..4}; do weka cluster drive add $i /dev/nvme0n1; weka cluster drive add $i /dev/nvme0n2; done
for i in {0..4}; do weka cluster drive add $i /dev/sdc --force; weka cluster drive add $i /dev/sdd --force; done
